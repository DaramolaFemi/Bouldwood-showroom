    ...
    <div className="container mx-auto px-6 py-12">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          />
          <button
            className="md:hidden px-3 py-2 border"
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-4 gap-8">
        <div className="hidden md:block">
          <FiltersSidebar
            materials={[
              ...new Set(all.flatMap((p) => (p.material ? [p.material] : []))),
            ]}
        ...
        <div className="md:col-span-3">
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
              {list.length === 0 && (
                <p className="col-span-full py-12">
                  No pieces match your search. Try another name or reset the
                  filters.
                </p>
              )}
              {list.map((p) => (
                <div key={p.id}>
                  <ProductCard
                    p={p}
                    onQuickView={(prod) => setQuick(prod.id)}
                  />
                </div>
              ))}
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div>
              <h3 className="text-xl font-display">
                {all.find((x) => x.id === quick)!.name}
              </h3>
              <p className="mt-4 text-muted">
                {all.find((x) => x.id === quick)!.description}
              </p>
              <div className="mt-6">
                <button
                  onClick={() => {
                    add(all.find((p) => p.id === quick)!);
                    setQuick(null);
                  }}
