...
export default function CartPage() {
  const { items, remove, updateQty, saved, moveToCart, saveForLater } =
    useCart();
        ...
        <div className="mt-6 grid md:grid-cols-3 gap-6">
          <div className="md:col-span-2">
            {items.map((i) => (
              <div
                key={i.product.id}
                className="cart-row flex items-center gap-4 py-4 border-b"
              >
                <img
                  src={`${i.product.images[0]}-360.jpg`}
                  alt={i.product.name}
                ...
                <div className="cart-controls flex items-center gap-2">
                  <input
                    ...
                    value={i.qty}
                    onChange={(e) =>
                      updateQty(i.product.id, Number(e.target.value))
                    }
          ...
          <aside className="p-6 border rounded">
            ...
            <div className="mt-4">
              <Link
                to="/checkout"
                className="block text-center px-4 py-3 bg-black text-white"
              >
                Checkout
              </Link>
            </div>
